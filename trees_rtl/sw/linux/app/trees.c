...
static int parse_run_mode(const char *mode_str, run_mode_t *mode)
{
    if (!strcasecmp(mode_str, "SW")) {
        *mode = RUN_MODE_SW;
        return 0;
    }
    if (!strcasecmp(mode_str, "HW")) {
        *mode = RUN_MODE_HW;
        return 0;
    }
    if (!strcasecmp(mode_str, "HWSW")) {
        *mode = RUN_MODE_HWSW;
        return 0;
    }
...
static int parse_binary_option(const char *value_str, int *value)
{
    char *endptr = NULL;
    long parsed;
    ...
    if (errno != 0 || endptr == value_str || *endptr != '\0' || (parsed != 0 && parsed != 1)) {
        return -1;
    }
...
static int parse_positive_option(const char *value_str, int *value)
{
    char *endptr = NULL;
    long parsed;
    ...
    if (errno != 0 || endptr == value_str || *endptr != '\0' || parsed < 1 || parsed > INT_MAX) {
        return -1;
    }
...
void evaluate_model(token_t *trees_buf, token_t *features_buf[MAX_BURST],  struct feature *features, int read_samples, int n_classes,
                    uint8_t *predictions, uint32_t max_burst, int load_trees_hw, int hw_iterations,
                    float *exe_time_ms_total, float *exe_time_ms_avg)
{
    float exe_t;
    uint8_t *iter_predictions_tmp = NULL;

    *exe_time_ms_total = 0.0f;

    if (hw_iterations > 1) {
        iter_predictions_tmp = (uint8_t *) malloc(max_burst * sizeof(uint8_t));
        if (iter_predictions_tmp == NULL) {
            printf("Error allocating temporary prediction buffer\n");
            *exe_time_ms_avg = 0.0f;
            return;
        }
    }
    ...
    if (load_trees_hw) {
        ...
        printf("Skipping tree loading on hardware (load_trees_hw=0)\n");
    }

    for (int iter = 0; iter < hw_iterations; iter++) {
        uint32_t processed = 0;
        uint32_t burst_i = 0;
        float iter_time_ms = 0.0f;

        //printf("HW iteration %d/%d\n", iter + 1, hw_iterations);

        while (processed < (uint32_t) read_samples) {
            uint32_t burst = ((uint32_t) read_samples - processed) > max_burst
                                ? max_burst
                                : ((uint32_t) read_samples - processed);
            uint8_t *pred_dst = (iter == 0) ? &predictions[processed] : iter_predictions_tmp;

            perform_inferences_hw(features_buf[burst_i],
                                  (int) burst, pred_dst, &exe_t);
            iter_time_ms += exe_t;
            processed += burst;
            burst_i++;
        }

        *exe_time_ms_total += iter_time_ms;
        // printf("  > Hardware iteration %d time: %f ms\n", iter + 1, iter_time_ms);
    }

    *exe_time_ms_avg = *exe_time_ms_total / (float) hw_iterations;
    printf("  > Hardware accumulated time (%d iter): %f ms\n", hw_iterations, *exe_time_ms_total);
    printf("  > Hardware average time: %f ms\n", *exe_time_ms_avg);

    free(iter_predictions_tmp);
...
int main(int argc, char **argv)
{
    ...
    float exe_time_ms_hw_total;
    float exe_time_ms_sw;
    int load_trees_hw = 1;
    int hw_iterations = 1;
    run_mode_t run_mode = RUN_MODE_HWSW;
    ...
    // dataset, modelo y parametros opcionales
    // [SW|HW|HWSW] [load_trees_hw:0|1] [hw_iterations>=1]
    printf("HOLA");
    if (argc < 3 || argc > 6) {
        printf("Use: %s <dataset.csv> <modelo.model> [SW|HW|HWSW] [load_trees_hw:0|1] [hw_iterations>=1]\n", argv[0]);
        return 1;
    }

    if (argc >= 4) {
        if (parse_run_mode(argv[3], &run_mode)) {
            printf("Modo invalido %s. Use SW, HW o HWSW\n", argv[3]);
            return 1;
        }
    }

    if (argc >= 5) {
        if (parse_binary_option(argv[4], &load_trees_hw)) {
            printf("Valor invalido para load_trees_hw: %s. Use 0 o 1\n", argv[4]);
            return 1;
        }
    }

    if (argc >= 6) {
        if (parse_positive_option(argv[5], &hw_iterations)) {
            printf("Valor invalido para hw_iterations: %s. Use un entero >= 1\n", argv[5]);
            return 1;
        }
    }
    ...
    if (run_mode == RUN_MODE_SW || run_mode == RUN_MODE_HWSW) {
    ...
    if (run_mode == RUN_MODE_HW || run_mode == RUN_MODE_HWSW) {
        printf("evaluate_model hardware\n");
        printf("HW config: load_trees_hw=%d, hw_iterations=%d\n", load_trees_hw, hw_iterations);
        if (run_mode == RUN_MODE_HWSW && hw_iterations > 1) {
            printf("HWSW mismatch check uses HW predictions from iteration 1\n");
        }
        evaluate_model(tree_buf, features_buf, features_read, read_samples,
                       n_classes, predictions_hw, MAX_BURST, load_trees_hw, hw_iterations,
                       &exe_time_ms_hw_total, &exe_time_ms_hw);
    }

    if (run_mode == RUN_MODE_HWSW) {
